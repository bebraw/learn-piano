...
- Under ADR-065, completion separately offers one immediate repeat when the current evaluator summary contains any pitch or order correction or at least one early or late timed interval. This prompt uses no retained history and does not replace or alter the advisory next-study suggestion.
...
- Full score notation, note-duration and velocity evaluation, rests, tuplets, syncopation, simultaneous chord events, hands-together coordination, adaptive tempo, pause/resume, richer history, and quality-sensitive or goal-sensitive study recommendations belong to later slices. The bounded current-attempt repeat cue does not make the recommender quality-sensitive.
...
- **Restart rule:** Restart stops active click guidance, creates a clean evaluator and timing anchor for the same exercise revision and selected tempo, clears transient feedback, immediate repeat guidance, and progress, and keeps already completed history. A timed repeat keeps the chosen tempo but returns pulse guidance to ready instead of starting it automatically. Restarting an incomplete attempt does not persist it as completed.
...
- **Immediate-repeat boundary:** A pure client projection reads only the current `EvaluationCompletionSummary`. It returns no cue for a clean untimed completion or a timed completion whose assessed intervals are all on time. Pitch or order corrections and non-zero early or late counts produce factual repeat copy plus the existing restart action, labelled with the retained BPM for timed studies. The cue adds no attempt field, repository read, aggregate, threshold, diagnosis, evaluator mutation, or recommendation input.
...
- Immediate repeat guidance is expressed as factual status text next to the renamed restart action. It does not hide the suggested study or imply failure, diagnosis, mastery, or a mandatory next step.
...
- If retained history already contains corrected or off-time attempts, those aggregates do not create a repeat cue after a clean current completion. Conversely, a current corrected or off-time completion may offer a repeat even when persistence fails because the cue does not claim durability.
...
- Do not derive immediate repeat guidance from retained history, newest-five aggregates, curriculum prerequisites, or the recommendation result, and do not let it hide or rewrite that result.
...
- [ ] A completed attempt with any pitch or order correction or any early or late timed interval shows one factual page-local repeat cue; clean untimed and all-on-time timed completions do not.
...
- [ ] Immediate repeat guidance retains the selected BPM in a timed action, clears through the ordinary restart path without automatically starting pulse guidance, and leaves the independent next-study suggestion visible and unchanged.
...
- Restart must clear transient evaluation and immediate-repeat state while preserving completed history and, for timed studies, the selected tempo.
...
- Immediate repeat guidance must not hide the recommendation, consult repository history, persist another field, or change evaluator, pulse, recommendation, or completion results.
...
- Recent evidence must neither create nor suppress the immediate repeat cue, which belongs only to the just-completed in-memory summary.
...
**Scenario: Repeat a corrected or off-time completion once**

- Given: the just-completed attempt contains a pitch or order correction or at least one early or late timed interval
- When: completion renders alongside the advisory next-study suggestion
- Then: a factual repeat cue relabels the existing restart action, preserves that suggestion, and restarting clears the cue and progress; a timed repeat retains its selected tempo while pulse guidance returns to ready

**Scenario: Finish cleanly without repeat pressure**

- Given: an untimed attempt has no pitch or order corrections, or a timed attempt has no corrections and every assessed interval is on time
- When: completion renders
- Then: no immediate repeat cue appears and the ordinary restart action and next-study suggestion remain available
