...
- Each C-major ordered chord-tone study declares only its matching untimed step-and-skip study as a prerequisite. Each D-minor study declares only its matching C-major chord-tone study, asks for D-F-A-F-D over D-E-F-G-A, and follows the C-major pair in canonical recommendation priority. All four remain freely selectable; their chord-tone and quality names do not imply simultaneous, blocked-chord, chord-quality-recognition, or harmony evaluation.
...
- [ ] `/practice` returns the canonical default, and `?exercise=<id>` selects each of the twenty-eight validated exercises without requiring client JavaScript.
...
- [ ] The C-major chord-tone pair retains five event and staff occurrences while reusing C and E keyboard controls over C-G; the D-minor pair does the same for D and F over D-A while keeping E and G idle and preserving the evaluator's normal wrong-note path.
...
- Then: the default C-D-E-F-G right-hand instructions, expected notes, and twenty-eight-exercise chooser remain visible, and history says JavaScript is required instead of claiming the history is empty
