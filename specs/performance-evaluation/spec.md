...
- The paired steady broken-chord studies evaluate C-E-G-E-C-E-G-E as eight individual occurrences at offsets 0–7. Each returning C, E, and G advances only when its distinct expected event becomes next; the first C is the ungraded MIDI anchor and the seven later accepted notes are classified only against their canonical gaps from it.
...
- **Audio boundary:** Under ADR-060, Web Audio derives count-in length, running-beat wrap, and beat-1 click accents from canonical timing metadata outside the evaluator. Fourteen studies use four count-in beats in 4/4; the 3/4 pair uses three count-in beats and wraps after beat 3. The six regular half-beat-grid studies ask the learner to place every “and” count halfway between clicks; the offbeat pair anchors C on the instructed downbeat and targets four successive “and” counts; the steady broken-chord pair asks for one note per click across two four-beat groups; and the 3/4 pair groups C-E-G-C-E-G-C as `1 2 3, 1 2 3, 1`. Audio times, wall-clock receipt times, latency estimates, meter labels, and click accents are never inputs to timing classification. A globally phase-shifted offbeat, steady broken-chord, or 3/4-loop performance can therefore retain the same MIDI-relative classifications and cannot prove audible phase, downbeat, between-click placement, learner accent, grouping, or measure alignment.
...
- Event feedback may identify the repeated pitch and the note still expected. Completion calls non-canonical repetitions `extra repeats` so the summary cannot be confused with intentional repeated events in the exercise.
...
- Completing a steady broken-chord study proves only eight accepted pitches in order and seven timing gaps relative to the first accepted MIDI note. It must not be described as proof of audible phase or measure alignment, duration, release, legato, rests, articulation, dynamics or velocity quality, fingering, declared-hand use, relaxation, harmony recognition, staff reading, consistency, or mastery.
- Completing a 3/4 broken-chord loop proves only seven accepted pitches in order and six whole-beat timing gaps relative to the first accepted MIDI note. It must not be described as proof of audible phase, downbeat, click or measure alignment, 3/4 counting or grouping, beat-1 accent or dynamics, duration, release, legato, rests, fingering, declared-hand use, relaxation, harmony recognition, staff reading, consistency, or mastery.
...
- Do not promote onset spacing, missing numbered-beat targets, chord-derived pitch order, or a two-measure count into audible phase or measure-alignment, duration, release, holding, legato, dynamics or velocity-quality, rest, silence, accent, articulation, notation, syncopation, harmony-recognition, simultaneous-chord, hands-together, or adaptive-tempo evidence.
...
- [ ] Timed mode accepts 40–100 BPM for all sixteen current studies, defaults to 60 BPM, and assesses each post-anchor interval against canonical beat gaps. The ten five-event timed studies assess four intervals, the 3/4 pair assesses six, and the mixed-pattern and steady broken-chord pairs each assess seven. The eight steady-quarter studies use inclusive ±0.2-beat tolerance, with the 3/4 pair extending integer offsets through beat 6 and the steady broken-chord pair through beat 7; the ascending even-eighth and repeated-note studies use inclusive ±0.1-beat tolerance at offsets 0, 0.5, 1, 1.5, and 2; the mixed-pattern pair extends the regular half-beat grid through beat 3.5; and the offbeat pair uses offsets 0, 0.5, 1.5, 2.5, and 3.5 with the same ±0.1-beat tolerance.
...
- Steady broken-chord timing must remain MIDI-relative and must never be presented as proof that its first note aligned with the audible pulse, that its later notes aligned with clicks, or that the eight onsets aligned with audible measure boundaries.
- The 3/4 pair's timing must remain MIDI-relative and must never be presented as proof that its first C aligned with audible beat 1, its later events followed audible clicks or three-beat groups, the learner counted 3/4, or any onset carried a performed accent.
...
**Scenario: Accept adjacent repeated-note occurrences**

- Given: `repeated-note-eighths-c-major-right-hand` expects C4-C4-D4-D4-E4 as five occurrence IDs on the half-beat grid
- When: the learner plays those pitches at 500 ms intervals after the first C4 anchors a 60 BPM attempt
- Then: both C4 and both D4 events are accepted as correct, all four assessed intervals are on time, and no repeated-input error is recorded

**Scenario: Classify an extra repeat after a canonical pair**

- Given: both C4 occurrences are accepted and D4 is next in the repeated-note study
- When: the learner plays another C4
- Then: it is `repeated`, D4 remains next, and the timing anchor and assessed-interval count do not change

**Scenario: Evaluate the mixed-pattern onset sequence**

- Given: `mixed-eighth-pattern-c-major-right-hand` expects C4-E4-D4-D4-F4-G4-E4-C4 at beat offsets 0 through 3.5 in half-beat steps
- When: the learner plays those eight pitches at 500 ms intervals after the first C4 anchors a 60 BPM attempt
- Then: all eight occurrences are accepted, all seven assessed intervals are on time, and completion claims no evidence about duration, release, articulation, fingering, declared-hand use, relaxation, reading, or consistency

**Scenario: Evaluate a steady broken-chord sequence**

- Given: `steady-quarter-broken-chord-c-major-right-hand` expects C4-E4-G4-E4-C4-E4-G4-E4 at beat offsets 0–7
- When: those eight pitches are accepted at one-second intervals after the first C4 anchors a 60 BPM attempt
- Then: all eight occurrences are accepted, all seven assessed intervals are on time within the ±0.2-beat window, and completion claims only ordered-pitch and MIDI-relative onset-gap evidence

**Scenario: Shift a steady broken chord away from the audible phase**

- Given: two steady-broken-chord fixtures have identical MIDI timestamp gaps but different first-note timestamps relative to Web Audio guidance
- When: both are evaluated with the same exercise revision and tempo
- Then: their seven timing classifications are identical, and neither result claims audible phase or measure alignment, duration, release, legato, rests, articulation, dynamics or velocity quality, fingering, hand use, relaxation, harmony recognition, staff reading, consistency, or mastery

**Scenario: Evaluate a 3/4 broken-chord loop**

- Given: `three-four-broken-chord-c-major-right-hand` expects C4-E4-G4-C4-E4-G4-C4 at beat offsets 0–6
- When: those seven pitches are accepted at one-second intervals after the first C4 anchors a 60 BPM attempt
- Then: all seven occurrences are accepted, all six assessed intervals are on time within the ±0.2-beat window, and completion claims only ordered-pitch and MIDI-relative whole-beat-gap evidence

**Scenario: Keep 3/4 guidance outside evaluation**

- Given: two 3/4-loop fixtures have identical MIDI timestamp gaps but different first-note timestamps relative to the three-beat count-in and accented beat-1 click
- When: both are evaluated with the same exercise revision and tempo
- Then: their six timing classifications are identical, and neither result claims audible phase, downbeat, click or measure alignment, 3/4 counting or grouping, learner accent, dynamics, duration, fingering, hand use, harmony recognition, reading, consistency, or mastery
