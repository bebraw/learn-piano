...
The home folio contains twenty-six freely selectable studies. That breadth supports both hands and several curriculum tracks, but scanning every card makes it harder to choose a relevant study. The learner needs a calm way to narrow the existing folio without turning parallel curriculum tags into exclusive categories, saved preferences, or progression gates.
...
- **Filter state:** Focus and hand compose as an intersection. All/All is the default on every page load, and Reset filters restores that state.
...
- A polite status reports `Showing N of 26 studies`; Reset filters remains explicit and is disabled at All/All.
...
- Without JavaScript, the controls stay hidden and all twenty-six links, titles, instructions, and completion-independent content remain usable.
...
- [ ] `/` retains all twenty-six canonical links in server HTML and renders hidden enhancement controls for three curriculum focuses plus right- and left-hand narrowing.
...
- [ ] Focus and hand filters compose without changing canonical order, card content, completion markers, recommendations, or practice routes.
...
- All/All must remain the initial state after navigation or reload, regardless of browser-restored radio values.
...
- A selected focus plus hand means both conditions, not either condition.
...
- **Client tests:** Cover neutral initialization, stale form-state reset, focus/hand composition, count copy, unknown-control fallback, and explicit reset.
- **Server tests:** Prove all canonical cards and focus metadata remain in HTML while controls begin hidden.
- **Browser tests:** Prove focus and hand counts, reset, reload default, badge preservation, storage-failure independence, responsive containment, and the complete no-JavaScript fallback.
...
**Scenario: Compose focus and hand**

- Given: the folio contains right- and left-hand Rhythm & coordination studies
- When: the learner selects Rhythm & coordination and Right
- Then: only right-hand rhythm studies remain visible and the status reports the matching count
...
- Then: All/All is selected, all twenty-six studies are visible, and no preference has been stored
