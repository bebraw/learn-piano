...
The repository has evolved from a runnable template stub into a personal piano-practice application while retaining the lightweight Cloudflare Worker baseline. The shell must serve a useful server-rendered twenty-eight-exercise library with a progressively enhanced local-practice overview and transient folio filtering, a stable health route, and same-origin generated assets without turning practice into a client framework application.
...
- **Routes:** `/` renders the application overview, local-practice enhancement shell, and twenty-eight-exercise library; `/practice` renders the canonical default; `/practice?exercise=<id>` renders an exact library selection; `/api/health` returns stable JSON; and unknown paths or supplied exercise IDs return a non-indexable 404 document.
...
- [ ] `/` returns the current Piano Practice overview and visible links to all twenty-eight canonical exercises.
...
- [ ] Direct D-minor selections server-render D-F-A-F-D as five event/staff occurrences over the D-E-F-G-A keyboard range with E and G idle, a supported A pitch, and no timing controls or blocked-chord claim.
...
- `GET /` must return HTML with the Piano Practice heading and visible links to all twenty-eight exercises.
...
- Then: they see the Piano Practice overview, an honest browser-history fallback, and can open any of the twenty-eight beginner exercises
...
- Then: the default exercise title, instructions, notes, twenty-eight-exercise chooser, and limitation text remain visible
