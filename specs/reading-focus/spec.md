...
- [ ] Physical-key focus, pressed state, operability, staff progress, numeric progress, count-in, pitch-free rhythm guidance, timing behavior, and completion remain intact; the steady broken-chord pair retains its generic one-note-per-beat task across eight positions, the 3/4 pair retains its three-beat count-in and full `1 2 3, 1 2 3, 1` task across seven, and the offbeat pair retains its full `1 & 2 & 3 & 4 &` task.
