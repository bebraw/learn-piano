...
- **Rhythm-guidance boundary:** Reading Focus may hide pitch-bearing exercise instructions but never the separate pitch-free rhythm task. The steady broken-chord study retains `Steady pulse`, `Pitch order · One note per beat`, and a task that applies one note per beat across its eight positions. The offbeat study remains playable from its staff order plus the visible instruction to place the first note on 1 and the remaining notes on successive “and” counts.
...
- [ ] Physical-key focus, pressed state, operability, staff progress, numeric progress, count-in, pitch-free rhythm guidance, timing behavior, and completion remain intact; the steady broken-chord pair retains its generic one-note-per-beat task across eight positions, and the offbeat pair retains its full `1 & 2 & 3 & 4 &` task.
