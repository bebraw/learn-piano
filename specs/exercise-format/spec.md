...
- The validated library contains thirty-three schema-version-1, revision-1 exercises:
  ...
  - `beethoven-ode-to-joy-opening-right-hand`: E4-E4-F4-G4-G4-F4-E4-D4
  - `pachelbel-canon-ground-bass-left-hand`: C3-G3-A3-E3-F3-C3-F3-G3
  - `bach-invention-1-opening-motif-right-hand`: C4-D4-E4-F4-D4-E4-C4-G4
- Twenty-two studies contain five expected-event occurrences; the 5/4 pulse pair contains six; the 3/4 broken-chord pair contains seven; and seven studies contain eight—the mixed-pattern and steady broken-chord pairs plus all three repertoire arrangements. The first twelve exercises, the D-minor five-note ascent pair, and the offbeat pair contain five distinct pitches; the four ordered chord-tone studies plus the repeated-note, steady broken-chord, and 3/4 broken-chord pairs each contain three distinct pitches; the 5/4 pair returns to its first C across six event IDs and five distinct C-position pitches; and every eight-event study preserves all occurrences while reusing its bounded physical-key span. Fifteen studies use `untimed-ordered-notes`; eighteen use `timed-ordered-notes`.
...
- Every exercise has learner-facing title and instructions, explicit difficulty, hand and prerequisite metadata, curriculum tags, and source metadata. The original thirty foundations remain beginner project material; the three repertoire arrangements use beginner, intermediate, and advanced task-relative labels and complete public-domain provenance.
...
- **Source and rights metadata:** Each exercise declares whether it is original, public domain, licensed, or user-provided, plus attribution or license information when applicable. Every `public-domain` source requires composer attribution, original work title, a rights note, an HTTPS reference URL without credentials, and an adaptation note. A repertoire-goal tag is motivational metadata and never grants permission to embed the referenced composition.
...
- [ ] Thirty-three stable exercises preserve the thirty original foundations—fifteen per hand—across ascending and descending motion, untimed and timed step-skip coordination, ordered C-major and D-minor chord tones, D-minor five-note ascents, straight steady quarter notes, steady broken-chord patterns, 3/4 broken-chord loops, 5/4 pulse patterns, even-eighth onset subdivision, adjacent repeated-note onsets, one longer mixed pattern, and offbeat-onset preparation, then append three sourced public-domain learning arrangements while preserving the original right-hand ascent as default.
- [ ] Twenty-two exercises contain five expected-event occurrences; the 5/4 pair contains six; the 3/4 pair contains seven; and seven exercises contain eight—the mixed-pattern and steady broken-chord pairs plus the three repertoire arrangements. Fifteen exercises are untimed and exactly eighteen use the timed evaluator.
...
- Protected repertoire goals remain metadata-only. Launchable repertoire requires independently encoded lawful source content and the complete public-domain provenance contract from ADR-064.
...
- **Library tests:** Protect stable identities, default selection, canonical order, valid prerequisite references, and acyclic prerequisites; validate all source metadata and per-exercise event counts; prove left-hand, descending, untimed step-skip, ordered chord-tone, straight steady-quarter, timed step-and-skip, even-eighth, repeated-note, eight-event mixed-pattern, offbeat-step-skip, eight-event steady broken-chord, seven-event 3/4 broken-chord, and six-event 5/4 pulse coverage; and protect the exact three repertoire sequences, levels, prerequisites, sources, and omissions.
...
- Given: thirty-three schema-version-1 documents for the current exercises
- When: the exercise library validates them
- Then: it exposes thirty-three unique stable IDs, preserves the thirty original foundations, adds three independently encoded public-domain arrangements, reports twenty-two five-event, two six-event, two seven-event, and seven eight-event studies, contains fifteen untimed and eighteen timed studies, validates every source contract, and keeps the original right-hand ascent as default
