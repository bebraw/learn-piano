...
- The validated library contains eighteen schema-version-1, revision-1 exercises:
...
- Every non-mixed study contains five expected-event occurrences; each mixed-pattern study contains eight. The first twelve exercises contain five distinct pitches; the ordered chord-tone and repeated-note studies each contain three distinct pitches, while the mixed pair returns to five distinct C-position pitches across eight event IDs. Eight studies use `untimed-ordered-notes`; the four steady-quarter, two ascending even-eighth, two repeated-note, and two mixed-pattern studies use `timed-ordered-notes`.
...
- A separate rhythm presentation helper derives learner labels from evaluation mode and canonical beat-offset spacing. All six half-beat-grid studies are labelled `Eighth-note grid` in the catalog and setup surface and `Pitch order · Even eighth-note onsets` beside the staff. Count guidance derives the complete grid from canonical event count and meter, wrapping numbered beats at each measure boundary; the current five-event studies end on count 3 while the eight-event studies show `1 & 2 & 3 & 4 &`. These presentation labels are not canonical exercise identity or added notation semantics.
...
- [ ] Eighteen stable exercises cover nine studies per hand, ascending and descending motion, untimed and timed step-skip coordination, ordered C-major chord tones, straight steady quarter notes, even-eighth onset subdivision, adjacent repeated-note onsets, and one longer mixed pattern while preserving the original right-hand ascent as default.
- [ ] Every non-mixed exercise contains five expected-event occurrences and the mixed-pattern pair contains eight. Eight exercises are untimed and exactly ten use the timed evaluator.
...
- [ ] The four steady-quarter studies define beat offsets 0–4 and ±0.2-beat tolerance; the two ascending even-eighth and two repeated-note studies define five offsets through beat 2, and the two mixed-pattern studies define eight half-beat offsets through beat 3.5. All six half-beat-grid studies use ±0.1 beat, and all ten timed studies use 40–100 BPM with 60 BPM default, 4/4 quarter-note pulse, and a four-beat count-in.
...
- The four steady-quarter, two ascending even-eighth, two repeated-note, and two mixed-pattern studies must remain hands-separate, single-note onset material unless a later revision and updated contract explicitly broaden them. Fractional spacing and longer phrase length must not acquire duration, release, articulation, fingering, hand-verification, relaxation, reading, consistency, rest, notation, or simultaneity meaning implicitly.
...
- **Library tests:** Protect stable identities, default selection, canonical order, valid prerequisite references, and acyclic prerequisites; validate all source metadata and per-exercise event counts, and prove left-hand, descending, untimed step-skip, ordered chord-tone, straight steady-quarter, timed step-and-skip, even-eighth, repeated-note, and eight-event mixed-pattern coverage.
...
- Given: eighteen schema-version-1 documents for the current exercises
- When: the exercise library validates them
- Then: it exposes eighteen unique stable IDs, five occurrences for every non-mixed study, eight for each mixed-pattern study, eight untimed and ten timed studies, lawful original-source metadata, and the original right-hand ascent as default
