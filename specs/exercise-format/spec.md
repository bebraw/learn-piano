...
- The validated library contains twenty-six schema-version-1, revision-1 exercises:
...
- Eighteen studies contain five expected-event occurrences; the 5/4 pulse pair contains six; the 3/4 broken-chord pair contains seven; and the mixed-pattern and steady broken-chord pairs each contain eight. The first twelve exercises and the offbeat pair contain five distinct pitches; the ordered chord-tone, repeated-note, steady broken-chord, and 3/4 broken-chord studies each contain three distinct pitches; the 5/4 pair returns to its first C across six event IDs and five distinct C-position pitches; and the mixed pair returns to five distinct C-position pitches across eight event IDs. Eight studies use `untimed-ordered-notes`; eighteen use `timed-ordered-notes`.
...
- Given: twenty-six schema-version-1 documents for the current exercises
- When: the exercise library validates them
- Then: it exposes twenty-six unique stable IDs, five occurrences for eighteen studies, six for each 5/4 pulse study, seven for each 3/4 broken-chord study, eight for each mixed-pattern and steady broken-chord study, eight untimed and eighteen timed studies, lawful original-source metadata, and the original right-hand ascent as default
