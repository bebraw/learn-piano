...
- Keep staff presentation behind a reversible adapter that derives the current pitch-only guide from canonical exercise events. Server-render the guide with an adjacent semantic text fallback, reject unsupported notation instead of guessing it, and do not let rendered SVG coordinates, glyphs, or state become exercise data or evidence of staff-reading mastery.
