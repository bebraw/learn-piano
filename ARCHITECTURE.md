...
- Under ADR-065, derive immediate repeat guidance only from the current page's just-completed evaluator summary. Offer one repeat when that attempt contains a pitch or order correction or any early or late timed interval, retain the selected tempo for a timed restart without starting audio automatically, and clear the cue on restart. Never persist or aggregate the cue, infer a diagnosis, or feed it into evaluation, history, curriculum, or study recommendation; keep the advisory next-study result visible and unchanged.
- Under ADR-061, keep home-folio discovery as a transient presentation over canonical exercise hand, every matching curriculum-track prefix, and evaluation mode. Compose those three facets as an intersection, server-render the complete library, default to the unrestricted All/All/All view on each load, preserve multi-track membership and canonical order, and never feed filter state into attempts, history, recommendations, evaluation, URLs, or progression claims.
...
- Keep protected repertoire names as goal and competency metadata. Launch public-domain repertoire only as independently encoded, bounded learning arrangements with explicit composer, work, rights, reference-source, and adaptation metadata under ADR-064; do not copy third-party notation, MIDI, recordings, editorial fingering, or other modern-edition material.
