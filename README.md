...
- After a completion with any pitch or order correction, or with any early or late timed interval, receive one calm `Repeat once` cue based only on that just-finished attempt. Timed repeats keep the selected tempo but return the pulse to ready instead of starting it automatically; a clean completion shows no repeat cue.
...
Full score notation, note duration, velocity, rests, syncopation, compound and further irregular meter, simultaneous chord events, hands-together work, adaptive tempo, quality-sensitive study recommendations, protected repertoire content, cloud sync, and iPad release distribution remain separate decisions for later slices.
