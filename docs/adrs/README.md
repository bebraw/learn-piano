...
| [ADR-025](./implemented/ADR-025-skip-agent-ci-for-docs-only-changes.md)                              | Superseded  | Allowed documentation-only changes to skip local Agent CI when executable behavior was unchanged.    |
...
| [ADR-028](./implemented/ADR-028-use-incremental-local-mutation-gate.md)                              | Superseded  | Used incremental Stryker runs in the local quality gate while GitHub CI ran full mutation.           |
...
| [ADR-048](./implemented/ADR-048-use-a-platform-neutral-midi-input-boundary.md)                       | Implemented | Isolate Web MIDI, native MIDI, and test input behind one normalized input port.                      |
...
| [ADR-061](./implemented/ADR-061-keep-home-folio-filtering-transient.md)                              | Implemented | Keep home folio focus, hand, and timing filters page-local, server-first, and inclusive.             |
...
| [ADR-066](./implemented/ADR-066-calibrate-mutation-break-threshold.md)                               | Implemented | Use a 65% mutation break floor while retaining the 80% target and browser-independent scope.         |
