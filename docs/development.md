...
Mutation testing uses Stryker with Vitest and the TypeScript checker. `npm run mutation` performs a full mutation run against browser-independent runtime `src/**/*.ts` files while excluding declarations, unit tests, end-to-end tests, `src/test-support.ts`, and the Playwright-owned `src/client/main.ts` composition entry. `npm run mutation:incremental` enables Stryker incremental mode so repeated deep local runs can reuse previous mutant results while still producing a complete mutation report. The Vitest runner uses Stryker's per-test coverage analysis and related-test narrowing, so each mutant runs against the tests Stryker can associate with the mutated file instead of blindly rerunning the whole suite. Stryker worker concurrency is set to `50%` so local mutation testing can use parallel workers without assuming a fixed core count or monopolizing a developer machine. The isolated GitHub mutation job overrides this with `--concurrency 100%` to use the runner fully and shorten the workflow critical path. Mutation reports and Stryker incremental data are written under `reports/`, and Stryker's temporary `.stryker-tmp/` sandbox must stay untracked.
